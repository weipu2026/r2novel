...
#   ① 语法：全仓库 node --check（SYNTAX_OK · 50 files；易漂移计数以门禁实际输出为准）
...
**判据不是「创建 vs 更新」，而是「本轮是否新建了分片」（saveOnce 的 `fresh` 脏片；2026-09-18 复查更正——
旧版这里写的二分法与代码不符，照它实现会在拆片场景复刻 P1-1）**：

> 任何多对象写入，失败后的状态必须落在**自愈链已覆盖**的那一侧。落盘顺序的选择依据不是
> 「哪个更符合直觉」，而是「**哪一侧有自愈代码**」。

两种情形的正确写序**相反**，别照抄：
- **本轮新建了分片**（首片 / 拆片 / missing 片重建，`shardRaw[si] == null`）：**新片 → 既有片 → root**
  （数据先、指针后）—— 新片写失败 ⇒ root 与既有片都没动，等于本次没发生；root 写失败 ⇒ 盘上分片数
  > root.shards，指针**落后**于盘，预筛走 `rebuildIdxRoot` 按盘重建，自愈覆盖
- **只改既有片**（分片布局不变，含大多数「更新」）：**root 先、数据后**（指针先）—— 数据写失败时
  指针指着的仍是旧内容，等于本次没生效，对账（`reconcileIdx`）可自愈。
  ⚠️ 这条**不等于**「更新一律指针先」：更新路径若伴随拆片/首书进新片，同样走上面的「新片先」序
