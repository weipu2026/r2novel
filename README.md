...
- **阅读状态筛选行**：`全部 / 未读 N / 在读 N / 已读完 N`（与分类导航叠加生效）——书多了也能一眼捞出「哪本看完了 / 哪本还没开」
- 五种排序（最近阅读 / 最近更新 / 创建时间 / 书名 / 字数）+ **置顶**、书名/标签即时搜索
- **阅读状态角标**（`读到 x/y 章` / `✓ 已读完`，未读不显示角标）、完结徽标、总字数统计、分页（60/页）
...
| GET `/api/books` | 书架摘要（pinned/标签/字数/完结/阅读状态 readDone/进度镜像） |
| POST `/api/books` | 建书（同名 → `duplicate:true`，客户端引导 替换/追加/副本） |
| GET / PATCH / DELETE `/api/books/:id` | 章节表 / 改书名·作者·标签·备注·置顶·完结·**已读完标记（readDone 三态）** / 软删进回收站 |
...
| `meta/<bookId>.json` | 单书元数据（章节表/作者/备注/标签/置顶/完结/已读完/cleanVer/孤儿章节表） |
